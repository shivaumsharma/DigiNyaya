"""Vendored, UNMODIFIED official Indian Kanoon API client.

Source: https://api.indiankanoon.org/static/ikapi.py (fetched 2026-07-18).
Indian Kanoon publishes this as the reference Python client for their API and
explicitly recommends using it directly or subclassing it, rather than a
hand-rolled HTTP client -- see https://api.indiankanoon.org/documentation/.

Do NOT edit this file. Compatibility fixes (e.g. the Cloudflare user-agent
issue already discovered in scripts/ingest_judgments.py) belong in a subclass
in the script that imports this module, not here -- see
scripts/source_eval_judgments.py::_IKApiWithBrowserUA.
"""
